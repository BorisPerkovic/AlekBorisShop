...
const nameValidation = (name) => {
  if (name !== "" && name.split("").every((element) => element !== parseInt(element))) {
    return name;
  } else {
    return null;
  }
...
const passwordValidation = (password) => {
  let counter = 0;
  if (password.split("").some((element) => element === element.toUpperCase())) {
    counter++;
  }
...
const emailValidation = (email) => {
  if ( email.split("").filter((element) => element === "@").length === 1 && email.split("@")
...
const showMessageRegistration = () => {
  ...
  successDiv.innerHTML = `Registration complete! Welcome ${regName.value}!`;
  mainSection.appendChild(successDiv);
};

export { nameValidation, emailValidation, passwordValidation, showMessageRegistration };
